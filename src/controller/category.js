...
import { CategoryAddSchema, updateCategorySchema } from "../schema/category";
...
export const updateCategory = async (req, res) => {
  ...
  try {
    ...
    const idCate = await Category.findById(id);
    ...
    if (checkName) {
      return res.status(400).json({
        message:
          "Tên danh mục đã tồn tại  Vui Lòng Chọn Những Tiêu chuẩn nghĩa khác nhau",
      });
    }
    ...
    const category = await Category.findByIdAndUpdate({ _id: id }, dataCate, {
      new: true,
    });
    if (!category || category.length == 0) {
      return res.status(400).json({
    ...
    return res
      .status(200)
      .json({ message: "Sửa danh mục thành công", category });
  } catch (error) {
    return res.status(500).json({
      message: error.message || "Lỗi server",
    });
