...
import { BrandAddSchema, BrandUpdateSchema } from "../schema/brand";
...
export const updateBrand = async (req, res) => {
  ...
  try {
    const { error } = BrandUpdateSchema.validate(
      {
        _id,
        brand_name,
        brand_image,
    ...
    const checkName = await Brand.findOne({ brand_name: req.body.brand_name });
    if (checkName) {
      return res.status(400).json({
        message:
    ...
    const exitSlug = slugify(req.body.brand_name, { lower: true });
    ...
    if (!newBrand)
      return res.status(500).json({ message: "Cập nhật thương hiệu thất bại" });

    return res
      .status(200)
      .json({ message: "Cập nhật thương hiệu thành công!", brand: newBrand });
  } catch (error) {
